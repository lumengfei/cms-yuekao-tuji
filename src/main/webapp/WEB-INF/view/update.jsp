...
<script type="text/javascript">
	...
	<form:form action="update" method="post" modelAttribute="student">
        <table class="table"><form:hidden path="id"/>
        	<tr>
        		<th>姓名</th>
        	...
        	<tr>
        	 ...
        	 <tr>
        		<th>所属省</th>
        		<th>
        		 <form:select path="provinces.id" class="provinces"  onchange="provinceschange($(this).val())">
        			<c:forEach items="${provinces}" var="p">
        			<option value="${p.id}" 
        			<c:if test="${p.id==student.provinces.id}">selected</c:if>
        			>${p.name}</option>
        			</c:forEach>
        			</form:select> 
        	...
        	<tr>
        		...
        		<th><form:select path="city.id" class="city" onchange="citychange($(this).val())">
        			...
        			<option value="${p.id}" 
        			<c:if test="${p.id==student.city.id}">selected</c:if>
        			>${p.name}</option>
        			</c:forEach>
        			</form:select>
        	...
        	<tr>
        		...
        		<th><form:select path="county.id" class="county">
        			<c:forEach items="${cityc}" var="p">
        			<option value="${p.id}" 
        			<c:if test="${p.id==student.county.id}">selected</c:if>
        			>${p.name}</option>
        			</c:forEach>
        			</form:select>
        			<form:errors path="name"></form:errors>  </th>
        	...
        	<tr>
        	 ...
        	 <tr>
        		...
        		<c:forEach items="${student.courses}" var="u">
        		<c:if test="${co.id==u.id}">checked</c:if>
        		</c:forEach>
        		 value="${co.id}">${co.name}&emsp;
        		</c:forEach>
